...
const FoodsGame: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  const [target, setTarget] = useState(FOODS[0]);
  const [options, setOptions] = useState<typeof FOODS>([]);
  ...
  return (
    <div className="bg-slate-800/50 p-4 rounded-lg text-center relative animate-fade-in">
      ...
      <h3 className="text-2xl font-bold mb-4">ما اسم هذا الطعام؟</h3>
