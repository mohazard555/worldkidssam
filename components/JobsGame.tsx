...
const JobsGame: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  const [target, setTarget] = useState(JOBS[0]);
  const [options, setOptions] = useState<typeof JOBS>([]);
  ...
  return (
    <div className="bg-slate-800/50 p-4 rounded-lg text-center relative animate-fade-in">
      ...
      <h3 className="text-2xl font-bold mb-4">ما اسم هذه المهنة؟</h3>
