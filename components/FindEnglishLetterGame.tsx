...
const FindEnglishLetterGame: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const generateNewRound = () => {
    setFeedback(null);
    const shuffled = shuffleArray(ENGLISH_LETTERS);
  ...
  return (
    <div className="bg-slate-800/50 p-4 rounded-lg text-center relative animate-fade-in" dir="ltr">
      <button onClick={onBack} className="absolute top-3 right-3 text-white/70 hover:text-white bg-black/20 p-2 rounded-full transition-colors z-10">
        <ArrowRightIcon className="w-6 h-6 transform -scale-x-100" />
        <span className="sr-only">Back</span>
      </button>
      <h3 className="text-2xl font-bold mb-4">Where is the letter...</h3>
...
export default FindEnglishLetterGame;
