...
const ArabicAlphabetFlashcards: React.FC = () => {
    const { appData } = useContext(AppContext)!;
    const arabicAlphabetData = appData.settings.arabicAlphabetData || [];
    const [selectedLetter, setSelectedLetter] = useState<AlphabetFlashcard | null>(null);

    if (selectedLetter) {
        return (
            <div className={`relative animate-fade-in w-full aspect-[4/3] max-w-2xl mx-auto rounded-2xl shadow-2xl p-6 flex flex-col items-center justify-center text-white ${selectedLetter.color}`}>
                <button 
    ...
    return (
        <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-7 lg:grid-cols-8 xl:grid-cols-10 gap-4">
            {arabicAlphabetData.map((letterData) => (
                <button 
                    ...
                    aria-label={`بطاقة حرف ${letterData.letter}`}
                >
...
export default ArabicAlphabetFlashcards;